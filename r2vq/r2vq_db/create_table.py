    ...
    for recipe in recipes:
        ...
        for ce in recipe.cooking_events:
            ...
            if rel:
                event = [rel.event.id]
                ingre_par = (
                    [r.id for r in rel.ingre_participants]
                    if rel.ingre_participants
                    else [None]
                )
                tool_par = (
                    [r.id for r in rel.tool_participants]
                    if rel.tool_participants
                    else [None]
                )
                habitat_par = (
                ...
                ingre_res = (
                    [r.id for r in rel.ingre_results] if rel.ingre_results else [None]
                )
                combinations = product(
                    event, ingre_par, tool_par, habitat_par, ingre_res
                )
            else:
            ...
            for combination in combinations:
                ...
                if pred:
                    ...
                    relation.update(
                        {
                            k: pred.__dict__[f"_{k}"].id
                            if pred.__dict__[f"_{k}"]
                            else None
                            for k in srl_keys[1:]
                        }
                    )
...
if __name__ == "__main__":
    create_tables("../r2vq_conllu_data/trial_all_formatted_corrected.csv")
